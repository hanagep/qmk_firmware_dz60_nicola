 ...
 * along with this program.  If not, see <http://www.gnu.gnu.org/licenses/>.
 */
/*
...
#include "nicola.h"
// #include "../key_duration/key_duration.h" // Commented out to disable timer
#include <timer.h>
...
// This variable will be declared globally by nicola.h
// static int key_process_guard = 0; // Removed static
int key_process_guard = 0; // Declared globally
...
void set_nicola(uint8_t layer) {
  nicola_layer = layer;
#ifdef TIMEOUT_INTERRUPT
  // keypress_timer_init(keypress_timer_expired); // Commented out to disable timer
#endif
}
...
void nicola_m_type(void) {
    switch(nicola_m_key) {
        ...
        case NG_QUOT: send_string(SS_TAP(X_BSPC)); break;
...
bool process_nicola(uint16_t keycode, keyrecord_t *record) {
  ...
  if (record->event.pressed) {
    if(NG_M_TOP <= keycode && keycode <= NG_M_BOTTOM) {
        ...
        // keypress_timer_start(TIMEOUT_THRESHOLD * 16); // Commented out to disable timer
        cont_process = false;
